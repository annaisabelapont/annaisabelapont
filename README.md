# 🔎 Sobre mim

📖 Estudante de Sistemas de Informação na Universidade de São Paulo (USP) e Técnica em Informática pelo Instituto Federal de Rondônia (IFRO)

🪪 Desenvolvedora Front-End e Designer UI/UX

💻 TypeScript, ecossistema Vue (Vue.js, Vue Router, Nuxt.js) e React (React.js, Next.js)

💨 Metodologias Ágeis (Scrum) e ferramentas (Kanban, Trello, Jira, GitHub Projects)

🔭 Experiência:

- **(2023-2025)**
  Bolsista nos projetos de pesquisa Sistema Gerador de Horário Acadêmico (SISGHA) e Sistema de Gestão de Ambientes (SISGEA)
  
- **(2025-2026)**
  Dev front-end junior no Projeto de Informatização Escolar (Proinfe)

<br />

## 📫 Contato
[![Gmail](https://img.shields.io/badge/Gmail-red?style=for-the-badge&logo=gmail&logoColor=white)](mailto:annaisabelapont@gmail.com)
[![Instagram](https://img.shields.io/badge/Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/annaisabelap)
